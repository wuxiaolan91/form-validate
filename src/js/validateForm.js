(function () {
  ...
  */
  ...
  var cfg = null
  var validateTip = ''
  var isTelarea = true // 随便写的
  var validateObj = {
    isAddress: function (address) {
      return /^[0-9a-zA-Z\u4e00-\u9fa5]{2,25}$/.test(address);
    },
    isChinese: function(str) { // 验证是不是中文字符
      return /^[\u4e00-\u9fa5]+$/i.test($.trim(str));
    },
    isBankNumber: function(str) { // 是不是银行卡卡号
      return /\d{16,19}/.test(str);
    },
    isPassword: function(str) { // 是否是密码
    ...
    /**
      ...
      */
    isPasswordOther: function(str) { // 是否是密码Other
    ...
    * @return {[number]}     [传过来的字符串的长度(汉字算2个字符,英文1个字符)]
    */
    strLen: function(str) {
      var len = 0
    ...
    isUserName: function(userName) {
      return /^[\u4e00-\u9fa5]{2,10}$/.test(userName);
    },
    isCompanyName: function(Companyname) {
      var len = Companyname.length,
      name = Companyname.split('');
      for (var i = 0; i < len; i++) {
        if (name[i] == '（' || name[i] == '）' || name[i] == '(' || name[i] == ')' || name[i] == '.' || name[i] == '-' || name[i] == '&' || (name[i] >= 'A' && name[i] <= 'Z') || (name[i] >= 'a' && name[i] <= 'z') || this.isChinese(name[i])) {} else {
          return false;
        }
      }
      return true;
  ...
  */
  var checkRule = function(elem) {
    ...
    $tip = $('#globalTip'),
    // 这个输入项的错误提示元素
    validateRule = $(this).data('validate'),
    // 这里面不同的验证规则应该是用逗号分隔的
    isVisible = $(this).is(':visible'),
    // 如果页面有用户隐藏的表单元素,那么不对它进行表单校验.
    isValidatePass = true;

    if (!validateRule || !isVisible) return false;
    var validateArray = validateRule.split(':'),
    validateItemName = validateArray[0],
    //验证项的名称
    validateItemValue = validateArray[1],
    //验证项的规则
    ...
    if ($tip.length < 1) { // 如果该元素下没有错误提示元素，就创建一个
      $tip = $('<p id="globalTip" class="tip"><span></span></p>');
      $(document).append($tip);
    }
      ...
      for (var i = 0; i < validateList.length; i++) {
        ...
        if (validate == 'required') {
          ...
          if (!$.trim(value)) { //该元素为空
            ...
            // showTip(elem.attr('data-empty-msg')||tipContent);
            return false;
          }
        } else {
          if (!value) break;
        ...
        if(/address/.test(validate)){
          if(!validateObj.isAddress(value)){
            isThisValidate = false;
          }
        } else if (validate == 'password' && !validateObj.isPassword(value)) {
        ...
        } else if (/'trueName'/.test(validate)) {
  ...
  $(document).on('blur', '[data-validate]',
  function(event) {
    console.log('校验')
    if (!validate($(this))) {
      return false;
